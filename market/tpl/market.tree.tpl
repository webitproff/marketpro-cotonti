<!-- 
	...
	*
	* Filename: _ _ _ _ _ _ _ _ _ _ _ _ _ _ market.tree.tpl
	* Base path to the file: _ _ _ _ _ _ _ modules/market/tpl/market.tree.tpl
	* Recommended path to the file: _ _ _ _ themes/your-theme-name/modules/market/market.tree.tpl
	* Main business logic:_ _ _ _ _ _ _ _ _ modules/market/inc/market.functions.php
	*                     _ _ _ _ SEE: cot_build_structure_market_tree()
	* Назначение:
	*   Базовый (дефолтный) шаблон иерархического дерева категорий (структуры)
	*   товаров модуля Market. Вторичный шаблон — вызывается изнутри других
	*   шаблонов, чаще всего через вызов в шаблоне темы:
	*
	*     <div class="card-body">
	*       {PHP|cot_build_structure_market_tree('', '', 0, '')}
	*     </div>
	*
	*   Этот файл является заготовкой: можно скопировать его под своим
	*   именем (например market.tree.mynewcopytree.tpl) и вызвать
	*   отдельной копией дерева, не затрагивая стандартное:
	*
	*     {PHP|cot_build_structure_market_tree('', '', 0, 'mynewcopytree')}
	*
	...
	*   подключает файл market.tree.<template>.tpl (здесь template = ''
	*   — берётся market.tree.tpl). На каждом уровне собирает строки
	*   категорий, парсит блок CATS и возвращает готовый HTML уровня,
	*   который затем вкладывается в контейнер {ROW_SUBCAT} родительской
	*   категории — так формируется дерево вложенности.
	*
	...
	*   $template — часть имени файла шаблона ('' — использовать market.tree.tpl).
	*
	...
	*
	*   Особые случаи:
	*     Если у категории нет дочерних — ROW_SUBCAT пустой, кнопка сворачивания
	*     не выводится (условие IF {ROW_SUBCAT} ), а вместо числа
	*     с потомками показывается badge с ROW_COUNT.
	...
	*
	* Различия с market.tree.list.tpl:
	*   market.tree.list.tpl   — вариант для боковой колонки страницы списка
	*                            (шаблон template='list', доп. блокировка и
	*                            подпись "категорий в list-контексте").
	*   market.tree.tpl (этот) — базовый/универсальный вариант дерева для
	*                            любого места шаблона темы; шаблон по умолчанию,
	*                            когда template не передан или пуст.
	*   Оба шаблона реализуют одну и ту же логику и используют одинаковые
	*   теги ROW_* — отличается только разметка обёртки и наличие/отсутствие
	*   узкоспециальных надписей.
	*
