...
$productJSON = [
    ...
    "description" => strip_tags($item['fieldmrkt_desc'] ?: $item['fieldmrkt_text']),
//"image" => !empty($item['fieldmrkt_file']) ? [$item['fieldmrkt_file']] : [],
    "brand" => [
        "@type" => "Organization",
        "name" => Cot::$cfg['maintitle']
    ], 
    // Заглушка для изображения
    "image" => !empty($item['fieldmrkt_file']) ? [$item['fieldmrkt_file']] : ["https://abuyfile.com/attacher/market/10/att_683.webp"],
    // Заглушка для бренда
    "brand" => [
        "@type" => "Organization",
        "name" => Cot::$cfg['maintitle'] ?: "MyStore"
    ],
    "offers" => [
        "@type" => "Offer",
        "url" => $item['fieldmrkt_pageurl'],
        "priceCurrency" => $item['fieldmrkt_currency'] ?? "RUB",
        "price" => $item['fieldmrkt_costdflt'] ?? 0,
        "availability" => ($item['fieldmrkt_count'] > 0) ? "https://schema.org/InStock" : "https://schema.org/OutOfStock"
    ]
];
...
// Добавляем пробел после точки, если его нет (чтобы текст не сливался)
$descriptionText = preg_replace('/(?<!\s)\.(?!\s)/u', '. ', $descriptionText);

// Убираем лишние пробелы
$descriptionText = preg_replace('/\s{2,}/u', ' ', $descriptionText);
...
Cot::$out['head'] .= '<script type="application/ld+json">'
	 ...
	 */
/* 
// ----------------- Подготовка корректного description -----------------
$descriptionHTML = $item['fieldmrkt_desc'] ?: $item['fieldmrkt_text'];

// Используем DOMDocument для извлечения текста
$doc = new DOMDocument();
libxml_use_internal_errors(true);
$doc->loadHTML('<?xml encoding="utf-8" ?>' . $descriptionHTML);
libxml_clear_errors();
...
foreach ($doc->getElementsByTagName('body')->item(0)->childNodes as $node) {
    if ($node->nodeType === XML_ELEMENT_NODE) {
        ...
        if ($text !== '') {
            if ($descriptionText !== '') {
                $descriptionText .= '. '; // точка между блоками
            }
...
// ----------------- JSON-LD для товара с заглушками -----------------
$productJSON = [
    "@context" => "https://schema.org",
    "@type" => "Product",
    "name" => $item['fieldmrkt_title'],
    "description" => $descriptionText,
    "image" => !empty($item['fieldmrkt_file']) 
        ? [$item['fieldmrkt_file']] 
        : ["https://abuyfile.com/attacher/market/10/att_683.webp"],
    "brand" => [
        "@type" => "Organization",
        "name" => Cot::$cfg['maintitle'] ?: "MyStore"
    ],
    ...
    "offers" => [
        ...
        "priceCurrency" => $item['fieldmrkt_currency'] ?? "RUB",
        "price" => $item['fieldmrkt_price'] ?? 0,
        "availability" => ($item['fieldmrkt_count'] > 0) 
...
if ($body) {
    foreach ($body->childNodes as $node) {
        if ($node->nodeType === XML_ELEMENT_NODE || $node->nodeType === XML_TEXT_NODE) {
            ...
            if ($text !== '') {
                if ($descriptionText !== '') {
                    $descriptionText .= '. '; // точка + пробел между блоками
                }
                $descriptionText .= preg_replace('/\s+/', ' ', $text); // убираем лишние пробелы внутри блока
            }
        }
    }
}
