 ...
 *
 * Path:    modules/market/inc/market.list.php
 *
 * ============================================================
 * ДОКУМЕНТАЦИЯ ПО ФАЙЛУ market.list.php
 * ============================================================
 *
 * Назначение:
 *   Отображает список товаров модуля Market. Файл является основным
 *   обработчиком вывода каталога товаров: категории, подкатегории,
 *   поиск, сортировка, фильтрация по дополнительным полям и пагинация.
 *   Поддерживает специальные категории: 'all', 'system', 'unvalidated',
 *   'saved_drafts'. Обеспечивает мультиязычность через плагин i18n4marketpro
 *   и подсветку ключевых слов в результатах поиска.
 *
 * Основные параметры URL:
 *   c=<код категории>       — код текущей категории (пусто = все товары);
 *   s=<поле сортировки>     — поле для сортировки (без префикса 'fieldmrkt_');
 *   w=<направление>         — направление сортировки: asc или desc;
 *   sq=<запрос>             — поисковый запрос;
 *   search_in=<область>     — область поиска: title, full, pcod;
 *   ord[]=<поля фильтра>    — массив полей для фильтрации (без префикса);
 *   p[]=<значения фильтра>  — массив значений фильтра;
 *   d=<страница товаров>    — номер страницы списка товаров (пагинация);
 *   dc=<страница категорий> — номер страницы списка категорий.
 *
 * Логика работы:
 *   1. Импортирует параметры запроса и определяет текущую категорию.
 *   2. Выполняет проверку прав доступа в зависимости от типа категории
 *      (общая, специальная, конкретная).
 *   3. Настраивает параметры отображения: количество товаров на страницу,
 *      длину обрезки текста, сортировку (по умолчанию или из категории).
 *   4. Формирует SQL-условия для выборки товаров с учётом:
 *      - категории и подкатегорий;
 *      - статусов (опубликованные для обычных пользователей, все для админа);
 *      - даты публикации;
 *      - дополнительных полей (фильтры ord[]/p[]);
 *      - поискового запроса (по названию, полному описанию или артикулу);
 *      - мультиязычных переводов (плагин i18n4marketpro).
 *   5. Выполняет SQL-запросы для подсчёта общего количества товаров
 *      и получения списка товаров текущей страницы.
 *   6. Генерирует пагинацию (для товаров и категорий), хлебные крошки,
 *      мета-теги (заголовок, описание, ключевые слова).
 *   7. Обрабатывает подкатегории: выводит их список с иконками и количеством
 *      товаров (учитывая подкатегории).
 *   8. В цикле формирует теги товаров (LIST_ROW_*) через cot_generate_markettags()
 *      и выводит их в шаблон.
 *   9. Обрабатывает дополнительные поля структуры (категории) и товаров.
 *  10. Поддерживает подсветку ключевых слов в результатах поиска.
 *
 * Используемые классы и сервисы:
 *   MarketDictionary       — константы состояний товара (STATE_PUBLISHED, STATE_PENDING и др.);
 *   Cot::$db               — прямые запросы к базе данных;
 *   XTemplate              — шаблонизатор вывода;
 *   cot_pagenav()          — генерация постраничной навигации;
 *   cot_generate_markettags() — формирование тегов товара;
 *   cot_generate_usertags()   — формирование тегов пользователя;
...
/* global $L;
include cot_langfile('market', 'module'); */
const COT_LIST = true;
...
// Определяем таблицу переводов, если плагин активен
$i18n_table = '';
...
$maxItemRowsPerPage = (int) Cot::$cfg['market']['cat___default']['marketmaxlistsperpageincat'];
...
if ($maxItemRowsPerPage <= 0) {
    ...
    $maxItemRowsPerPage = Cot::$cfg['marketmaxlistsperpageincat'];
...
if (
    ...
    && !empty(Cot::$cfg['market']['cat_' . $c]['marketmaxlistsperpageincat'])
) {
    // Проверка, задана ли категория, существует ли конфигурация для этой категории и задано ли количество элементов на страницу
    $maxItemRowsPerPage = (int) Cot::$cfg['market']['cat_' . $c]['marketmaxlistsperpageincat'];
...
$marketCatpPath = [
    ...
    [cot_url('market'), $L['market_title_general']]
];
...
// подключение хуков для плагинов перед формированием SQL-запроса
...
if (!empty($sq)) {
    ...
    // Базовая часть условия (по умолчанию только основные поля)
    if ($search_in === 'title') {
    ...
    // Подключаем поиск по переводам, если активен плагин и язык не основной
    $use_i18n_search = false;
    $i18n_condition = '';

    if (cot_plugin_active('i18n4marketpro')) {
        // Регистрируем таблицу, если ещё не зарегистрирована
        if (!isset(Cot::$db->i18n4marketpro_pages)) {
            Cot::$db->registerTable('i18n4marketpro_pages');
        }
        $i18n_table = Cot::$db->i18n4marketpro_pages;
        ...
        if ($current_locale !== Cot::$cfg['defaultlang']) {
            ...
            // Формируем подзапрос EXISTS для поиска в переводах
            if ($search_in === 'title') {
            ...
            // Для 'pcod' перевод не ищем
        }
    }

    // Финальное условие WHERE для поиска
    if ($use_i18n_search && !empty($i18n_condition)) {
        $where['search'] = "($base_condition OR $i18n_condition)";
    } else {
        $where['search'] = $base_condition;
    }
}
...
// ========== новое 15-03-2026 ===============
// Добавляем параметры поиска в ссылки пагинации принудительно
$pagination_params = $list_url_path;
if (!empty($sq)) {
    $pagination_params['sq'] = $sq;
    $pagination_params['search_in'] = $search_in;
}
$pagination_params['dc'] = $dcurl;
$pagenav = cot_pagenav('market', $pagination_params, $d, $totallines, $maxItemRowsPerPage);
// ========== новое 15-03-2026 ===============

// Очистка и экранирование заголовка категории для безопасного вывода
$catTitle = htmlspecialchars(strip_tags($cat['title']));

// Очистка и экранирование описания категории (базовое)
Cot::$out['desc'] = htmlspecialchars(strip_tags($cat['desc']));

// --- Установка подзаголовка (subtitle) ---
Cot::$cfg['maintitle'] = ''; // очищаем, чтобы header.php не добавлял свой префикс
if (!empty($c) && !empty(Cot::$cfg['market']['cat_' . $c]['marketmetatitle'])) {
    Cot::$out['subtitle'] = Cot::$cfg['market']['cat_' . $c]['marketmetatitle'];
} else {
    Cot::$out['subtitle'] = $catTitle;
}

// --- Установка ключевых слов (keywords) ---
if (!empty($c) && !empty(Cot::$cfg['market']['cat_' . $c]['marketkeywords'])) {
    Cot::$out['keywords'] = Cot::$cfg['market']['cat_' . $c]['marketkeywords'];
} elseif (!empty(Cot::$cfg['market']['cat___default']['marketkeywords'])) {
    Cot::$out['keywords'] = Cot::$cfg['market']['cat___default']['marketkeywords'];
}

// --- Установка мета-описания (description) ---
if (!empty($c) && !empty(Cot::$cfg['market']['cat_' . $c]['marketmetadesc'])) {
    Cot::$out['desc'] = Cot::$cfg['market']['cat_' . $c]['marketmetadesc'];
} elseif (empty(Cot::$out['desc']) && !empty(Cot::$cfg['market']['cat___default']['marketmetadesc'])) {
    Cot::$out['desc'] = Cot::$cfg['market']['cat___default']['marketmetadesc'] . ' - ' . $catTitle;
}
...
$t->assign([
    ...
    'LIST_CAT_COUNT' => (int) ($cat['count'] ?? 0),
	
    // Экранированный заголовок категории
    'LIST_CAT_TITLE' => htmlspecialchars($cat['title']),
	'LIST_CAT_TITLE_LANG_LINE' => isset($L['market_list_cat_title_lang_line_' . $c]) 
		 ? htmlspecialchars($L['market_list_cat_title_lang_line_' . $c]) 
		 : '',
	'LIST_CAT_TITLE_LISTITEMS_SCHEMAORG_LANG_LINE' => isset($L['market_list_cat_title_ListItemschemaOrg_' . $c]) 
		 ? htmlspecialchars($L['market_list_cat_title_ListItemschemaOrg_' . $c]) 
		 : '',
...
// Назначение переменных шаблона для формы поиска  autofocus
$t->assign([
...
// ========== новое 15-03-2026 ===============
// Добавляем параметры поиска в ссылки пагинации подкатегорий
$pagination_cat_params = $list_url_path;
if (!empty($sq)) {
    $pagination_cat_params['sq'] = $sq;
    $pagination_cat_params['search_in'] = $search_in;
}
$pagination_cat_params['d'] = $durl;
$pagenav_cat = cot_pagenav('market', $pagination_cat_params, $dc, count($allsub), Cot::$cfg['market']['marketmaxlistsperpage'], 'dc');
...
if (!$sqllist_rowset_other) {
    ...
    foreach ($sqllist_rowset as $item) {
        ...
        $t->assign([
            ...
            'LIST_ROW_NUM' => $jj,
			'LIST_ROW_ABS_NUM' => $jj + $d,   // абсолютный номер с учётом пагинации
        ]);
